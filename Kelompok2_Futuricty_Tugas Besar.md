...
// Helper function untuk process single location
...
- **Axum & Tokio Runtime**: Menggunakan `axum` sebagai web framework yang ringan dan `tokio` sebagai async runtime untuk menangani request secara concurrent tanpa blocking.
- **Sequential Location Processing**: Menggunakan `stream::iter().fold()` untuk memproses locations satu per satu secara immutable, tanpa mutable variables, untuk menghindari rate limiting dari Overpass API.
- **Retry Mechanism with Exponential Backoff**: `process_location` helper function mengimplementasikan retry hingga 3 kali dengan delay yang meningkat eksponensial (5s, 10s, 20s). Menggunakan functional `stream::iter().fold()` untuk mencari first success attempt tanpa mutable state.
- **Controlled Concurrency per Location**: Di dalam `fetch_facilities` (overpass.rs), setiap location memproses kategori dengan maksimal 2 concurrent requests (`buffer_unordered(2)`), menjaga keseimbangan antara kecepatan dan rate limit.
- **Validation Pattern**: Validasi koordinat menggunakan for loop sederhana dengan early return untuk error handling yang jelas.
- **Deduplication State**: Struct `DeduplicationState` dengan metode immutable (`add_facility` return new state) menerapkan prinsip FP untuk menghindari mutasi langsung. Digunakan dalam `process_location` untuk deduplikasi facilities.
- **Config-Driven Queries**: Query Overpass dibuat dari config JSON menggunakan functional chain (`get().and_then().map()`) untuk fleksibilitas tanpa hardcode.
- **Inter-Location Delay**: Delay 3 detik antar locations untuk memberi waktu Overpass API recovery dari request sebelumnya.
- **Immutable Processing**: Semua operasi menggunakan functional patterns (`map`, `fold`, `filter`, `collect`) tanpa `let mut` - mengikuti functional programming paradigm.
...
- **Macro-based Code Generation**: Menggunakan macro `serde_clone!`, `serde_clone_default!`, dan `serde_only!` untuk mengeliminasi boilerplate code dan menerapkan DRY (Don't Repeat Yourself) principle.
- **Type Safety**: Struct yang strongly-typed memastikan data yang dikirim/diterima sesuai kontrak API, menghindari runtime error.
- **Automatic Serialization**: `serde` melakukan konversi otomatis antara Rust struct dan JSON tanpa kode manual, menjaga pure functional data transformation.
- **Separation of Concerns**: Request/Response DTOs terpisah dari domain models untuk menjaga clean architecture.
...
- **Retry Logic per Category**: `fetch_overpass_data_with_retry` melakukan retry hingga 3 kali per kategori dengan exponential backoff (2s, 4s, dst) untuk HTTP 429/504 errors.
- **Declarative Data Flow**: Menggunakan `stream::iter().map().buffer_unordered()` untuk memproses queries secara deklaratif tanpa loop imperatif.
- **Controlled Concurrency**: `buffer_unordered(2)` membatasi hanya 2 request concurrent per location. Dari 10 kategori, hanya 2 yang di-fetch bersamaan, sisanya menunggu. Strategi ini mencegah rate limiting sambil memaksimalkan throughput.
- **Rate Limiting**: Setiap request ke Overpass API menunggu 1000ms (1 detik) sebelum eksekusi untuk menghormati rate limit server.
- **Status-based Retry Decision**: Hanya retry untuk HTTP 429 (Too Many Requests) dan 504 (Service Unavailable), tidak untuk 400/401 errors.
- **Error Propagation**: Jika semua retry gagal untuk kategori, error di-return dengan informasi jelas tentang jumlah attempts.
- **Async/Await Pattern**: Semua operasi non-blocking, memaksimalkan throughput server tanpa thread blocking.

### 4. Score Calculator Service (`src/services/score_calculator.rs`)

Ini adalah modul inti yang berisi logika bisnis untuk menghitung jarak, kontribusi fasilitas, dan skor akhir.
...
- **Parallel Processing**: `par_iter()` dari Rayon membagi data ke multiple threads secara otomatis, memanfaatkan semua CPU cores untuk perhitungan jarak ribuan fasilitas.
- **Functional Composition**: Chain `filter_map()` menggabungkan filtering dan mapping dalam satu pass, lebih efisien dari pendekatan imperatif.
- **Distance Decay Formula**: Fungsi `calculate_contribution` menerapkan exponential decay dengan configurable weights untuk memberikan bobot lebih tinggi pada fasilitas yang dekat.
- **Immutable Aggregation**: `fold()` digunakan untuk akumulasi data tanpa mutasi external state, semua state baru dikembalikan sebagai tuple `(counts, map)`.
- **Config-Driven Logic**: Semua parameter (weights, decay factors, mappings) di-load dari JSON config, memisahkan business logic dari configuration.
- **Pure Functions**: Semua helper functions (`get_contribution_weights`, `normalize_distance`, dll) adalah pure - output hanya bergantung pada input, no side effects.
...
- **Pattern-Based Detection**: Menggunakan array of function pointers `[(Detector, &'static str); 10]` untuk mendefinisikan semua detector dalam struktur data, bukan if-else chain.
- **Functional Search**: `find()` method mencari detector pertama yang match, lalu `map()` mengekstrak category name - pattern yang elegant dan readable.
- **Config-Driven Patterns**: Semua pattern matching rules di-load dari `category_patterns.json`, memungkinkan update rules tanpa recompile code.
- **Pure Functions**: Setiap detector function (`is_education`, `is_health`, dll) adalah pure function yang hanya membaca input, tidak ada side effects atau global state mutation.
- **Lazy Static Config**: `PATTERN_CONFIG` di-load sekali saat startup menggunakan `Lazy`, menghemat I/O dan memory.
- **Option Type**: Return type `Option<&'static str>` secara eksplisit menyatakan kemungkinan "tidak ada kategori yang match" tanpa perlu null checks.
...
- **Module System**: File `mod.rs` adalah Rust's module system yang mengexpose submodules (`pub mod`) agar bisa di-import oleh `main.rs` menggunakan `use crate::services::*`.
- **Encapsulation**: Setiap service dipisah ke file tersendiri, menerapkan separation of concerns dan single responsibility principle.
- **Public Interface**: Hanya fungsi/struct yang di-mark `pub` yang visible ke luar module, sisanya tetap private untuk encapsulation.

### 7. Config Category Patterns (`config/category_patterns.json`)

File category_patterns.json ini berfungsi sebagai kumpulan aturan pencarian (search rules) yang menentukan bagaiman setiap titik/fasilitas dari OpenStreetMap diklasifikasikan ke dalam kategori tertentu seperti education, health, market, dan seterusnya.
...
- **Multi-Tag Matching**: Sistem mendukung kombinasi multiple tags untuk akurasi deteksi (contoh: `shop_non_empty` + `amenity_equals` untuk kategori market).
- **Localization Support**: Patterns mencakup bahasa Indonesia dan Inggris untuk mendeteksi nama fasilitas lokal (contoh: "rumah sakit", "rsud", "hospital").
- **Flexible Configuration**: Backend membaca JSON ini saat startup, memungkinkan update detection rules tanpa perlu recompile aplikasi.
- **Hierarchical Matching**: Menggunakan struktur nested (`category -> field -> values`) untuk organized dan maintainable rules.
...
- **Overpass QL Templates**: Mendefinisikan template query untuk Overpass API dengan placeholder untuk node/way types dan tag patterns yang akan di-inject saat runtime.
- **Element Type Separation**: Membedakan query untuk `node` (point location) dan `way` (area/polyline) untuk hasil yang lebih akurat dari OSM database.
- **Regex Pattern Support**: Tag values menggunakan regex (`hospital|clinic|doctors`) untuk mencocokkan multiple values dalam satu query, mengurangi jumlah API calls.
- **Config Key Convention**: Naming convention `node_amenity`, `way_shop`, dll memudahkan parsing dan query generation di fungsi `parse_config_key()`.
- **Centralized Query Management**: Semua query patterns terpusat di satu file, memudahkan maintenance dan update tanpa menyentuh business logic.
- **Default Settings**: Section `settings` menyediakan default values (radius 500m, output format JSON) yang digunakan jika tidak dispesifikasikan.
...
- **Contribution Weights**: Mengatur bobot kontribusi maksimal, decay factor (kecepatan penurunan kontribusi seiring jarak), dan minimum contribution ratio untuk setiap kategori fasilitas. Contoh: health memiliki `decay_factor: 0.7` yang lebih rendah dari transport `0.9`, artinya fasilitas kesehatan masih dianggap penting meski agak jauh.
- **Score Weights**: Menentukan persentase pengaruh setiap kelompok indikator terhadap skor akhir. Services (30%), Mobility (25%), Safety (25%), Environment (20%). Total harus 100% untuk scoring yang balanced.
- **Category Mappings**: Memetakan kategori detail (health, market, dll) ke dalam 4 kelompok indikator besar (services, mobility, safety, environment) untuk agregasi skor yang meaningful.
- **Name Extraction**: Fallback hierarchy untuk ekstraksi nama fasilitas - coba field `name` dulu, kalau tidak ada coba `amenity`, lalu `shop`, dst. Default ke "facility" jika semua field kosong.
- **Configurable Parameters**: Semua parameter dapat diubah tanpa recompile, memungkinkan fine-tuning scoring algorithm berdasarkan feedback pengguna atau domain expert.
